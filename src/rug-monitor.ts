...
app.post("/rug-alert", async (req: Request, res: Response) => {
  ...
  for (const tx of txs) {
    ...
    // Get parsed tx for instruction details (correct Helius method)
...
watching.forEach((watch, mint) => {
  ...
  pollInterval = setInterval(async () => {
    try {
      const poolBalance = await helius.rpc.getTokenAccountsByOwnerV2(new PublicKey(mint), { programId: new PublicKey("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA") });
